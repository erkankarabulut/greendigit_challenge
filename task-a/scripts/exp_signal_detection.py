"""
Signal detection experiments for Task A.1.
...
from task_a.evaluation import load_detections, score_detection
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", type=Path, default=ROOT / "task-a" / "outputs" / "exp_signal_detection.csv")
...
def run_clf_config(
    ...
    det_path = tmp_dir / f"det_{clf_backend}_{context_length}_{tag}.csv"
    detections = model.predict_detection(test, max_context_length=context_length)
    write_detections(detections, det_path)
    preds = load_detections(det_path)
    parts = score_detection(test, preds)
    elapsed = time.time() - t0
    return {
        "S_A1": parts["S_A1"],
        "auroc_a1": parts["auroc_a1"],
        "f1_a1": parts["f1_a1"],
...
def main() -> None:
    ...
    for backend in CLF_BACKENDS:
        ...
        try:
            ...
            fh.flush()
            print(f"  S_A1={result['S_A1']:.4f}  f1={result['f1_a1']:.4f}  ({result['elapsed_s']}s)")
    ...
    for backend in CONTEXT_SWEEP_BACKENDS:
        ...
        for ctx in CONTEXT_LENGTHS:
            ...
            t1 = time.time()
            try:
                det_path = args.tmp_dir / f"det_ctx_{backend}_{ctx}.csv"
                detections = model.predict_detection(test, max_context_length=ctx)
                write_detections(detections, det_path)
                preds = load_detections(det_path)
                parts = score_detection(test, preds)
                ...
                fh.flush()
                print(f"    S_A1={parts['S_A1']:.4f}  ({elapsed:.1f}s)")
            except Exception as exc:
                print(f"    ERROR: {exc}")
    ...
    try:
        ...
        _write_row(writer, result)
        fh.flush()
        print(f"  S_A1={result['S_A1']:.4f}  ({result['elapsed_s']}s)")
    except Exception as exc:
        print(f"  ERROR: {exc}")
